...
for host in $hostnames; do
  ...
  /usr/sbin/kadmin -p $user/admin add -r --use-defaults host/$host.ocf.berkeley.edu
  echo "Exporting host/$host.ocf.berkeley.edu keytab"
  rm -f $host/krb5.keytab
  /usr/sbin/kadmin -p $user/admin ext_keytab -k $host/krb5.keytab host/$host.ocf.berkeley.edu
done
